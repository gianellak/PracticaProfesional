...
public class UsuarioDB {
	...
	private static final String SQL_USERS = "SELECT * FROM Usuario";

	private static final String SQL_INSERT = "INSERT INTO Usuario VALUES (?, ?, ?, ?, ?, ?, ?)";

	private static final String SQL_INSERT_EMP = "INSERT INTO Empleado VALUES (?, ?, ?, ?, ?, ?, ?)";

	private static final String SQL_DELETE = "DELETE FROM Usuario WHERE username = ?";

	private static final String SQL_DELETE_EMP = "DELETE FROM Empleado WHERE idEmpleado = ?";

	private static final String SQL_FIND_BY_ID = "SELECT * FROM Usuario WHERE username=?";

	private static final String SQL_FIND_BY_DNI = "SELECT * FROM Usuario WHERE dniUsuario=?";

	private static final String SQL_FIND_EMP_BY_ID = "SELECT * FROM Empleado WHERE idEmpleado=?";

	private static final String SQL_UPDATE = "UPDATE Usuario SET username=?, password=?, permisos=?, nombre=?, apellido=?, bloqueo=? WHERE dniusuario=?";

	private static final String SQL_UPDATE_EMP = "UPDATE Empleado SET nombre=?, apellido=?, idCargo=?, direccion=?, telefono=?, email=? WHERE idEmpleado=?";
	...
	public boolean update(Usuario user) throws DBException {
		...
		Object[] values = { user.getUsername(), user.getPassword(),
				user.getPermisos(), user.getNombre(), user.getApellido(),
				user.getBloqueo(), user.getDniUsuario() };
	...
	public boolean updateEmpleado(Empleado e) throws DBException {
		...
		Object[] values = { e.getNombre(), e.getApellido(), e.getIdCargo(),
		...
		Connection connection = this.connectionProvider.getConnection();
				...
				PreparedStatement statement = DBUtil.prepareStatement(
						connection, SQL_UPDATE_EMP, false, values);) {
	...
	public boolean deleteEmpleado(int dni) throws DBException {
		...
		try (Connection connection = this.connectionProvider.getConnection();
				...
				PreparedStatement statement = DBUtil.prepareStatement(
						connection, SQL_DELETE_EMP, false, values);) {
			int affectedRows = statement.executeUpdate();
			if (affectedRows == 0) {
				throw new DBException(
						"Deleting empleado failed, no rows affected.");
	...
	public boolean insert(Usuario user) throws DBException {
		...
		Object[] values = { user.getUsername(), user.getPassword(),
				user.getDniUsuario(), user.getPermisos(), user.getNombre(),
				user.getApellido(), user.getBloqueo() };

		System.out.println(this.connectionProvider);

		try (Connection connection = this.connectionProvider.getConnection();
				PreparedStatement statement = DBUtil.prepareStatement(
						connection, SQL_INSERT, false, values);) {
			int affectedRows = statement.executeUpdate();
			if (affectedRows == 0) {
				throw new DBException(
	...
	public Usuario findDNIu(int idUser) throws DBException {
		...
		try (Connection connection = this.connectionProvider.getConnection();
				PreparedStatement statement = DBUtil.prepareStatement(
						connection, SQL_FIND_BY_DNI, false, values);
				ResultSet resultSet = statement.executeQuery();) {
			if (resultSet.next()) {
				user = map(resultSet);
			}
	...
	private static Usuario map(ResultSet resultSet) throws SQLException {
		Usuario user = new Usuario(resultSet.getString("username"),
				resultSet.getString("password"),
				resultSet.getInt("dniUsuario"), resultSet.getInt("permisos"),
				resultSet.getString("nombre"), resultSet.getString("apellido"),
				resultSet.getBoolean("bloqueo"));
	...
	public Empleado findIdEmpleado(int dni) throws DBException {
		...
		try (Connection connection = this.connectionProvider.getConnection();
				PreparedStatement statement = DBUtil.prepareStatement(
						connection, SQL_FIND_EMP_BY_ID, false, values);
				ResultSet resultSet = statement.executeQuery();) {
			if (resultSet.next()) {
	...
	public boolean inserE(Empleado e) throws DBException {
		...
		Object[] values = { e.getIdEmpleado(), e.getNombre(), e.getApellido(),
				e.getDireccion(), e.getIdCargo(), e.getTelefono(), e.getEmail()
